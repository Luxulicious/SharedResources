...
namespace TheLuxGames.SharedResources.UI
{
    [ExecuteAlways]
    public class RenderGrid : MonoBehaviour
    {
        ...
        void OnGUI()
        {
            ...
            foreach (var point in _pointsToDrawLabelsAt)
            {
                ...
                var viewPortPoint = Camera.main.WorldToViewportPoint(adjustedPoint);
                if (!(viewPortPoint.x > 0 && viewPortPoint.x < 1 && viewPortPoint.y > 0 &&
                      viewPortPoint.y < 1)) continue;
